...
def main():
    ...
    for md in memory_data:
        ...
        p05_ssnr.append(np.percentile(ssnr, 5))
        p25_ssnr.append(np.percentile(ssnr, 25))
        p50_ssnr.append(np.percentile(ssnr, 50))
        p75_ssnr.append(np.percentile(ssnr, 75))
    ...
    p05_ssnr = np.array(p05_ssnr); p25_ssnr = np.array(p25_ssnr)
    p50_ssnr = np.array(p50_ssnr); p75_ssnr = np.array(p75_ssnr); p95_ssnr = np.array(p95_ssnr)
    ...
    ax1.set_title(r"Memory amplitude $\hat{A}$, $\sigma_A$, $|\hat{A}/\sigma_A|$, and $\hat{A}/\sigma_A$ per event"
