...
exact event-level memory likelihood inputs used by `run_hierarchical_analysis.py`
for the `--analyze memory` run, then performs fast leave-one-event-out (LOO)
importance reweighting from the saved `result_memory.nc` posterior.

Outputs
-------
- event_influence.csv
    One row per event with LOO influence metrics and PE-derived properties.
- property_correlations.csv
    Spearman correlations between influence metrics and event properties.
- top_events_report.txt
    Human-readable summary of the most important events and the properties of the
    top precision-driving subset.
- top_precision_drivers.png
    Bar chart of the leading events ranked by how much they tighten the
    mu_tgr constraint.
...
def _load_run_inputs(
    ...
    memory_dir = memory_dir or Path(_read_first_line(results_dir / "memory_dir.txt"))
    ...
    name_to_post = {
        _event_name_from_path(f): p for f, p in zip(all_event_files, all_posteriors)
    }
    ...
    # Reconcile fallback PE-group selection back into the memory-data waveform.
    fixed_memory: List[dict] = []
    fixed_posteriors: List[np.ndarray] = []
    fixed_names: List[str] = []
    for name, md, post, fpath in zip(ordered_names, memory_data, mem_posteriors, mem_files):
    ...
    # Reconstruct the exact memory-model arrays used by the original run.
    ...
    # Reconstruct the final event-name list after the same IFAR cut.
    ...
    if analyzed_events_path.exists():
        ...
        ifars = load_event_ifars(
            ordered_names,
            str(ifar_cache_path) if ifar_cache_path.exists() else None,
        )
        pre_names = [n for n in ordered_names if ifars.get(n, float("inf")) >= cfg.ifar_threshold]
        rng_names = np.random.default_rng(cfg.seed)
        final_names = []
        for n in pre_names:
            md = next(md for md in memory_data if md["event_name"] == n)
            idxs = rng_names.choice(len(md["A_hat"]), size=cfg.n_samples_per_event, replace=True)
            if np.all(np.isnan(md["A_hat"][idxs])):
                continue
            final_names.append(n)
    if len(final_names) != nobs:
        raise RuntimeError(
            f"Reconstructed {len(final_names)} event names but generate_tgr_only_data returned Nobs={nobs}."
        )
    ...
    if cfg.scale_tgr and A_scale != 0:
        mu_model = mu / A_scale
        sigma_model = sigma / A_scale
    else:
        mu_model = mu.copy()
        sigma_model = sigma.copy()
...
def _event_properties(name: str, ps: np.ndarray, A_hat: np.ndarray, A_sigma: np.ndarray) -> Dict[str, float]:
    ...
    if m1 is not None and q is not None:
        ...
        for prefix, arr in [
            ...
            ("m2_source", m2),
            ("mtot_source", mtot),
            ("mchirp_source", mchirp),
            ("mass_ratio", q),
        ]:
            q05, q50, q95 = _q05_q50_q95(arr)
            out.update({f"{prefix}_q05": q05, f"{prefix}_median": q50, f"{prefix}_q95": q95})
    else:
        for prefix in ["m1_source", "m2_source", "mtot_source", "mchirp_source", "mass_ratio"]:
...
def _event_loglike_matrix(
    ...
    n_events, n_samples = A_hats.shape
...
def _rank_events(
    ...
    for j, name in enumerate(names):
        ...
        row = {
            ...
            "delta_logdet_cov": loo["logdet_cov"] - full["logdet_cov"],
    ...
    influence["rank_precision"] = influence["delta_mu_halfwidth68"].rank(ascending=False, method="dense").astype(int)
    influence["rank_shift"] = influence["abs_delta_mu_q50"].rank(ascending=False, method="dense").astype(int)
    influence["rank_area"] = influence["delta_logdet_cov"].rank(ascending=False, method="dense").astype(int)
    return influence.sort_values(["rank_precision", "rank_shift", "rank_area", "event_name"]).reset_index(drop=True)
...
def _property_correlation_table(df: pd.DataFrame) -> pd.DataFrame:
    metrics = ["delta_mu_halfwidth68", "abs_delta_mu_q50", "delta_logdet_cov"]
    props = [
        "snr_median",
    ...
    return out.sort_values(["metric", "spearman_rho"], ascending=[True, False]).reset_index(drop=True)
...
def _group_profile_lines(df: pd.DataFrame, top_names: Sequence[str]) -> List[str]:
    ...
    for col, label in props:
        ...
        lines.append(
            f"  - {label}: top median = {top_med:.3g}, rest median = {rest_med:.3g}, "
            f"top-median percentile among all events = {pct:.1f}%"
        )
    return lines


def _write_text_report(df: pd.DataFrame, corr_df: pd.DataFrame, outpath: Path, top_k: int) -> None:
    top_precision = df.nlargest(top_k, "delta_mu_halfwidth68")
    top_shift = df.nlargest(top_k, "abs_delta_mu_q50")
    top_area = df.nlargest(top_k, "delta_logdet_cov")
    overlap = sorted(set(top_precision["event_name"]) & set(top_shift["event_name"]))

    with open(outpath, "w") as f:
        f.write("Leave-one-out influence summary for the memory-only (mu_tgr, sigma_tgr) posterior\n")
        f.write("=" * 78 + "\n\n")

        f.write(f"Top {top_k} precision drivers (largest increase in mu_tgr 68% half-width when removed):\n")
        for _, row in top_precision.iterrows():
            f.write(
                f"  {row['event_name']}: Δhalfwidth={row['delta_mu_halfwidth68']:+.4g}, "
                f"Δmu_med={row['delta_mu_q50']:+.4g}, Δlogdet={row['delta_logdet_cov']:+.4g}, "
                f"SNR={row['snr_median']:.3g}, q={row['mass_ratio_median']:.3g}, "
                f"Mchirp={row['mchirp_source_median']:.3g}, chi_eff={row['chi_eff_median']:.3g}, "
                f"DL={row['luminosity_distance_median']:.3g}\n"
            )
        f.write("\n")

        f.write(f"Top {top_k} mean-pulling events (largest |shift| in mu_tgr median when removed):\n")
        for _, row in top_shift.iterrows():
            f.write(
                f"  {row['event_name']}: |Δmu_med|={row['abs_delta_mu_q50']:.4g}, "
                f"Δhalfwidth={row['delta_mu_halfwidth68']:+.4g}, Δlogdet={row['delta_logdet_cov']:+.4g}, "
                f"SNR={row['snr_median']:.3g}, q={row['mass_ratio_median']:.3g}, "
                f"Mchirp={row['mchirp_source_median']:.3g}, chi_eff={row['chi_eff_median']:.3g}\n"
            )
        f.write("\n")

        f.write(f"Top {top_k} contour-area drivers (largest increase in log det Cov[mu_tgr, sigma_tgr] when removed):\n")
        for _, row in top_area.iterrows():
            f.write(
                f"  {row['event_name']}: Δlogdet={row['delta_logdet_cov']:+.4g}, "
                f"Δhalfwidth={row['delta_mu_halfwidth68']:+.4g}, |Δmu_med|={row['abs_delta_mu_q50']:.4g}, "
                f"SNR={row['snr_median']:.3g}, q={row['mass_ratio_median']:.3g}, "
                f"Mchirp={row['mchirp_source_median']:.3g}\n"
            )
        f.write("\n")

        f.write("Overlap between the top precision drivers and top mean-pulling events:\n")
        f.write("  " + (", ".join(overlap) if overlap else "(none)") + "\n\n")

        f.write("Property profile of the precision-driving subset versus the rest:\n")
        for line in _group_profile_lines(df, list(top_precision["event_name"])):
            f.write(line + "\n")
        f.write("\n")

        f.write("Strongest property correlations with influence metrics (top 5 by |Spearman rho| for each metric):\n")
        for metric in ["delta_mu_halfwidth68", "abs_delta_mu_q50", "delta_logdet_cov"]:
            f.write(f"  {metric}:\n")
            sub = corr_df[corr_df["metric"] == metric].copy()
            sub["abs_rho"] = np.abs(sub["spearman_rho"])
            sub = sub.sort_values("abs_rho", ascending=False).head(5)
            for _, row in sub.iterrows():
                f.write(
                    f"    - {row['property']}: rho={row['spearman_rho']:+.3f} (n={int(row['n'])})\n"
                )
        f.write("\n")


def _make_bar_plot(df: pd.DataFrame, outpath: Path, top_k: int) -> None:
    plot_df = df.nlargest(top_k, "delta_mu_halfwidth68").sort_values("delta_mu_halfwidth68")
    fig, ax = plt.subplots(figsize=(10, max(4, 0.35 * len(plot_df))))
    ax.barh(plot_df["event_name"], plot_df["delta_mu_halfwidth68"])
    ax.set_xlabel(r"Increase in $\mu_A$ 68% half-width when event is removed")
    ax.set_ylabel("Event")
    ax.set_title("Leading precision-driving events in the memory-only analysis")
    fig.tight_layout()
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--results-dir", required=True, type=Path, help="Output directory from run_hierarchical_analysis.py")
    ...
    p.add_argument("--top-k", type=int, default=20, help="Number of top events to show in the report and plot")
...
def main() -> None:
    ...
    influence_df = _rank_events(names, mu, sigma, loglikes, properties_df)
    ...
    _make_bar_plot(influence_df, outdir / "top_precision_drivers.png", top_k=min(args.top_k, len(influence_df)))

    full = _summary_stats(mu, sigma)
    print(f"Reconstructed {len(names)} events from {args.results_dir}")
    print(
        f"Full posterior: mu_tgr median = {full['mu_q50']:.4g}, "
        f"68% half-width = {full['mu_halfwidth68']:.4g}, "
        f"sigma_tgr median = {full['sigma_q50']:.4g}"
    )
    print(f"Top precision driver: {influence_df.iloc[0]['event_name']}")
    print(f"Wrote: {outdir / 'event_influence.csv'}")
    print(f"Wrote: {outdir / 'property_correlations.csv'}")
    print(f"Wrote: {outdir / 'top_events_report.txt'}")
    print(f"Wrote: {outdir / 'top_precision_drivers.png'}")
